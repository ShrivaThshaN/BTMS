document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateAllData() {
        const data = [];
        const tagNames = ['TPM065', 'TVT066', 'TPM067', 'TJL068'];
  
        for (let i = 1; i <= totalData; i++) {
            const randomIndex = Math.floor(Math.random() * tagNames.length);
            const item = {
                'SID': i.toString(),
                'TagName': tagNames[randomIndex],
    ...
    updatePagination();
      ...
      window.location.href = 'DeltaT.html';
      });
